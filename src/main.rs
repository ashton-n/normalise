...
#[test]
fn test_main() {
    let input_dir = std::path::PathBuf::from("unit_test_data/");
    let output_filename = std::path::PathBuf::from("unit_test.dat");   
    ...
    let overall = std::path::PathBuf::from("unit_test_data/overall.dat");
    let unit_test = std::path::PathBuf::from("unit_test.dat");
    
    let test_data = std::fs::read(&overall).expect("Error reading test data");
    let unit_test_output = std::fs::read(&unit_test).expect("Error reading unit test output");

    // remove the test file
    std::fs::remove_file(&unit_test).expect("Error removing test file");

    assert_eq!(test_data, unit_test_output);
}
